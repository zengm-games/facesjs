...
for (const [_, itemList] of Object.entries(toolbarItemConfig)) {
  for (const itemConfig of itemList) {
    if (itemConfig.renderOptions && itemConfig.renderOptions.rangeConfig) {
      ...
      itemConfig.renderOptions.valuesToRender = generateRangeFromStep(
        rangeConfig.min,
        rangeConfig.max,
        rangeConfig.step,
      );
...
const createFaceSlace: StateCreator<CombinedState, [], [], FaceState> = (
  set: any,
) => ({
...
const createToolbarSlice: StateCreator<CombinedState, [], [], ToolbarState> = (
  set: any,
) => ({
