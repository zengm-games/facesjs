...
describe('generateRangeFromStep function tests', () => {
    ...
    it('throws an error for zero step', () => {
        expect(() => generateRangeFromStep(0, 5, 0)).toThrow('Step must be greater than 0');
    });

    it('throws an error for negative step', () => {
        expect(() => generateRangeFromStep(5, 10, -1)).toThrow('Step must be greater than 0');
    });

    it('generates a range in reverse if start is greater than end (with positive step)', () => {
        expect(() => generateRangeFromStep(10, 5, 1)).toThrow('Start cannot be greater than end when step is positive');
    });
